...
import {
  AlertTriangle, Bot, Braces, Check, ChevronDown, ChevronRight, CircleDot,
  ...
  Search, Sparkles, TerminalSquare, TestTube2, X,
} from "lucide-react";
...
type PanelTab = "Context" | "AI" | "Impact";
...
const INITIAL_FILES: WorkspaceFile[] = [
  {
    id: "user-service", name: "UserService.ts", path: "src/services/UserService.ts", language: "typescript", status: "clean",
...
export async function getUserName(id: string) {
  ...
  {
    id: "profile", name: "Profile.tsx", path: "src/components/Profile.tsx", language: "typescript", status: "clean",
    content: `import { getUserName } from "@/services/UserService";
...
export async function Profile({ userId }: { userId: string }) {
  ...
  {
    id: "profile-test", name: "Profile.test.ts", path: "src/tests/Profile.test.ts", language: "typescript", status: "clean",
    content: `import { getUserName } from "@/services/UserService";
...
describe("getUserName", () => {
  it("returns the API user name", async () => {
    ...
    await expect(getUserName("usr_123")).resolves.toBe("Aqib");
  });
});`,
  },
  { id: "checkout", name: "Checkout.tsx", path: "src/components/Checkout.tsx", language: "typescript", content: `export function Checkout() {\n  return <main>Checkout</main>;\n}` },
  { id: "header", name: "Header.tsx", path: "src/components/Header.tsx", language: "typescript", content: `export function Header() {\n  return <header>ShopX</header>;\n}` },
  { id: "order-service", name: "OrderService.ts", path: "src/services/OrderService.ts", language: "typescript", content: `export async function getOrder(id: string) {\n  return fetch(\`/api/orders/\${id}\`);\n}` },
  ...
  { id: "dashboard", name: "dashboard.tsx", path: "src/pages/dashboard.tsx", language: "typescript", content: `export default function Dashboard() {\n  return <div>Dashboard</div>;\n}` },
  { id: "profile-page", name: "profile.tsx", path: "src/pages/profile.tsx", language: "typescript", content: `export default function ProfilePage() {\n  return <div>Profile</div>;\n}` },
  { id: "user-service-test", name: "UserService.test.ts", path: "tests/UserService.test.ts", language: "typescript", content: `describe("UserService", () => {\n  it("loads a user", () => {});\n});` },
...
function ContextPanel({ tab, setTab, syncState, upstream, fixPreview, fixApplied, onPreview, onApply, onDismiss, onViewImpact, hidden, setHidden }: {
  ...
  return <aside className="flex h-full min-h-0 flex-col border-l border-white/[0.07] bg-[#101012]">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-3">
      ...
      {tab === "AI" && <div className="space-y-3"><div><SectionLabel>Sync AI</SectionLabel><h2 className="mt-1 flex items-center gap-1.5 text-sm font-semibold"><Sparkles className="size-3.5 text-foreground/70" /> Contract migration</h2></div>{fixApplied ? <div className="rounded-md border border-emerald-400/25 bg-emerald-400/[0.07] p-3"><p className="flex items-center gap-1.5 text-xs font-semibold text-emerald-100"><Check className="size-3.5" /> AI fix applied</p><p className="mt-1.5 text-[11px] text-emerald-100/70">Modified: UserService.ts, Profile.test.ts</p><Button size="xs" className="mt-2" onClick={() => document.getElementById("run-validation")?.click()}>Run validation</Button></div> : fixPreview ? <div className="space-y-2"><p className="text-[11px] leading-relaxed text-muted-foreground">The patch replaces the outdated response field and updates its contract test.</p><pre className="overflow-x-auto rounded-md border border-white/[0.08] bg-black/25 p-2.5 sc-mono text-[10px] leading-5"><span className="text-red-300">- return user.name;</span>{"\n"}<span className="text-emerald-300">+ return user.full_name;</span></pre><div className="grid grid-cols-3 gap-1.5 text-center text-[10px]"><span className="rounded border border-white/[0.07] bg-white/[0.025] px-1 py-1.5">2 files</span><span className="rounded border border-white/[0.07] bg-white/[0.025] px-1 py-1.5">1 test</span><span className="rounded border border-emerald-400/20 bg-emerald-400/[0.06] px-1 py-1.5 text-emerald-200">LOW risk</span></div><div className="flex gap-1.5"><Button size="xs" onClick={onApply}>Apply Fix</Button><Button size="xs" variant="outline" onClick={onDismiss}>Dismiss</Button></div></div> : <div className="rounded-md border border-white/[0.10] bg-white/[0.035] p-3"><p className="text-[11px] leading-relaxed text-foreground/75">{upstream ? "I detected an upstream API contract change. Your code still expects user.name; the new API returns user.full_name." : "No active contract drift detected. Sync AI is monitoring User API dependencies."}</p>{upstream && <><p className="mt-2 text-[11px] font-medium text-foreground/90">Suggested action: update the consumer and associated tests.</p><Button size="xs" className="mt-2" onClick={onPreview}>Preview fix</Button></>}</div>}</div>}
...
export function CodeWorkspace({ action }: { action?: string }) {
  ...
  const [activeId, setActiveId] = useState("user-service");
  const [openIds, setOpenIds] = useState(["user-service", "user-type", "profile"]);
  ...
  const [rightWidth, setRightWidth] = useState(286);
  ...
  const affected = files.filter((f) => f.status === "affected");
  const dirtyCount = changed.length;

  const openFile = useCallback((id: string) => { setActiveId(id); setOpenIds((p) => p.includes(id) ? p : [...p, id]); }, []);
  const updateContent = useCallback((value?: string) => {
    if (value === undefined) return;
    setFiles((p) => p.map((f) => f.id === activeId ? { ...f, content: value, status: "modified" } : f));
    if (!upstream && !fixApplied) setSyncState("MODIFIED");
  }, [activeId, fixApplied, upstream]);
  const simulateUpstream = useCallback(() => {
    setUpstream(true); setFixApplied(false); setFixPreview(false); setSyncState("OUT_OF_SYNC"); setContextTab("Context"); setValidation(0);
    setFiles((p) => p.map((f) => ["user-service", "profile", "profile-test"].includes(f.id) ? { ...f, status: "affected" } : f));
  }, []);
  const previewFix = useCallback(() => { setFixPreview(true); setContextTab("AI"); setSyncState("AI_FIX_READY"); }, []);
  ...
  const runValidation = useCallback(() => {
    ...
    [2, 3, 4].forEach((stage, index) => timers.current.push(window.setTimeout(() => { setValidation(stage as 2 | 3 | 4); if (stage === 4) setSyncState("VALIDATED"); }, 650 * (index + 1))));
  }, []);
  const save = useCallback(() => { if (syncState === "MODIFIED") setSyncState("SYNCED"); }, [syncState]);
  const resize = (side: "left" | "right", event: React.PointerEvent<HTMLDivElement>) => {
    const start = event.clientX; const initial = side === "left" ? leftWidth : rightWidth;
    const move = (e: PointerEvent) => { const delta = e.clientX - start; const next = Math.max(174, Math.min(360, initial + (side === "left" ? delta : -delta))); side === "left" ? setLeftWidth(next) : setRightWidth(next); };
  ...
  return <div className="-mx-3 -my-4 flex h-[calc(100svh-48px)] min-h-[620px] flex-col bg-[#0b0b0d] sm:-mx-5 sm:-my-5">
    <div className="flex min-h-10 items-center gap-2 border-b border-white/[0.07] bg-[#111113] px-3">
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="min-h-0 flex-1" style={{ display: "grid", gridTemplateColumns: `${filesHidden ? 34 : leftWidth}px 4px minmax(0, 1fr) 4px ${contextHidden ? 34 : rightWidth}px` }}>
        ...
        <div onPointerDown={(e) => !filesHidden && resize("left", e)} className="cursor-col-resize bg-white/[0.025] hover:bg-violet-300/50" />
        ...
        <div onPointerDown={(e) => !contextHidden && resize("right", e)} className="cursor-col-resize bg-white/[0.025] hover:bg-violet-300/50" />
    ...
    <div className="absolute bottom-2 left-3 z-20"><button onClick={simulateUpstream} className="flex items-center gap-1.5 rounded border border-white/[0.10] bg-[#171719] px-2 py-1.5 text-[10px] text-muted-foreground shadow-lg transition-colors hover:border-amber-300/30 hover:text-amber-100"><AlertTriangle className="size-3 text-amber-300" />Simulate upstream API change</button></div>
